...
static NSString * const FZZCreatorCellKind = @"CreatorCell";
static NSString * const FZZEngagedCellKind = @"EngagedCell";
static NSString * const FZZInvitedCellKind = @"InvitedCell";
...
@implementation FZZCellFlowLayout
...
- (CGRect)getTextRectFromEvent:(FZZEvent *)event{
    ...
    
    {
        ...
        if ([messages count] > 0){
            FZZMessage *message = [messages objectAtIndex:0];
            text = [message text];
        }
    }
...
- (CGRect)makeCreatorFrameFromTextBubbleRect:(CGRect)textBubbleRect{
    ...
    float width  = kFZZCreatorProfilePictureWidth;
    float height = kFZZCreatorProfilePictureHeight;
...
-(NSArray *)getArrayOfUniqueCommenters:(FZZEvent *)event{
    ...
    for (int j = 0; j < numMessages; j++){
        FZZMessage *message = [messages objectAtIndex:j];
...
-(CGRect)getCommentBubblePlacementFrameFromTextBubble:(CGRect)textBubbleRect{
    ...
    float maxHorizontalCommentJutOut = - (kFZZCommentProfilePictureWidth / 3.0);
    float maxVerticalCommentJutOut   = (2.0 * kFZZCommentProfilePictureHeight / 3.0);
...
-(NSArray *)attendeeFramesForPlacementFrame:(CGRect)commentBubblePlacementFrame{
    ...
    if (divisibleWidth / 3 < kFZZCommentProfilePictureWidth){
        divisibleWidth = commentBubblePlacementFrame.size.width - (minimumSpacing);
        
        if (divisibleWidth / 2 >= kFZZCommentProfilePictureWidth) {
            maxComments = 4;
        } else {
            divisibleWidth = commentBubblePlacementFrame.size.width;
            
            if (divisibleWidth >= kFZZCommentProfilePictureWidth){
                maxComments = 2;
            }
        }
    }
    ...
    float width = kFZZCommentProfilePictureWidth;
    float height = kFZZCommentProfilePictureHeight;
...
-(NSArray *)layoutUsersAroundBubble:(CGRect)commentBubblePlacementFrame
    ...
    for (int i = 0; (i < numEngagedUsers) && (i < numFrames - 1); i++){
        FZZUser *user = [engagedUsers objectAtIndex:i];
...
- (void)prepareLayout
{
    ...
    /*newLayoutInfo[FZZInvitedCellKind] = ;
    newLayoutInfo[FZZCreatorCellKind] = ;
    newLayoutInfo[FZZEngagedCellKind][indexPath] = ;*/
